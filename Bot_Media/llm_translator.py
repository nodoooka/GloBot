...
async def translate_batch(segments: list, ocr_results: list) -> list:
    ...
    try:
        response = await active_client.chat.completions.create(
            model=active_model,
            # 开启 JSON 约束模式 (DeepSeek / OpenAI 均兼容此参数)
        ...
        logger.error(f"❌ 批量翻译 JSON 解析崩溃: {e}")
        raise RuntimeError(f"LLM_TRANSLATION_FAILED: 批量翻译格式损坏 - {e}")