...
2. 配置密钥与词典
复制环境模板文件：cp .env.example .env

在 .env 中填入你的 Telegram Bot Token、个人 Chat ID 以及 LLM 的 API Key。

检查 config.yaml，调整你要监控的偶像推特 ID 及作息时间。

运行 python Bot_Media/knowledge_example_init.py 自动生成初始 RAG 知识库。
